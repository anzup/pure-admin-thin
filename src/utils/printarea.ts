export default class {
  ...
  init() {
    ...
    if (this.settings.asyncUrl) {
      _this.settings.asyncUrl(function (url) {
        const PrintAreaWindow = _this.getPrintWindow(url) // 创建iframe
  ...
  removeCanvasImg() {
    ...
    try {
      if (_this.elsdom) {
        // 删除canva转变图片的dom节点
        const canvasList = _this.elsdom.querySelectorAll('.canvasImg')
  ...
  Iframe(url) {
    ...
    const iframe = this.iframeBox(frameId, url)
