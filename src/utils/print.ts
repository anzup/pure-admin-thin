...
export function toPdf(fileData) {
  if (!fileData.data) {
    return
  }
  ...
  console.log(href)
  iframe.id = 'myIframe'
  iframe.style = 'position: fixed;width: 60%;height: 800px;top: -1000px;right: 60px;'
  document.body.appendChild(iframe)
  w = f.contentWindow || f.contentDocument
  doc = f.contentDocument || f.contentWindow.document
  doc.open()

  const pdfUrl = 'pdf/web/viewer.html?file=' + encodeURIComponent(href)
  iframe.src = pdfUrl
