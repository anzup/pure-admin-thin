...
<script setup lang="ts">
  ...
  import { getStatisticsUseRatio } from '/@/api/opm/statistical'
  ...
  const state = reactive({
    title: t('text.facility_utilization'),
    gridOptions: {
      columns: [
    ...
    getData: (params: { year: string; resourceId: number }) => {
      ...
      getStatisticsUseRatio(params).then((res) => {
        chartList.value = res.data.useRatios || []
        state.gridOptions.data =
          res.data.useRatios?.length > 0
            ? [
                ...res.data.useRatios,
                {
