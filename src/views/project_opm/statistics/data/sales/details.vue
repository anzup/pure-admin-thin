...
<script setup lang="ts">
  ...
  const state = reactive({
    ...
    gridOptions: {
      columns: [
        { field: 'customerName', title: t('text.customer_name'), width: 150, fixed: 'left' },
        ...Array.from(new Array(12), (v, index) => {
          return {
            title: t(monthList[index]),
            children: [
              {
                field: `wetAmounts[${index}]`,
                title: t('text.wet_lease'),
                formatter: firmatCount,
                minWidth: 100,
              },
              {
                field: `dryAmounts[${index}]`,
                title: t('text.dry_lease'),
                formatter: firmatCount,
                minWidth: 100,
              },
            ],
          ...
          res.data.incomes.length > 0
            ? [
                ...res.data.incomes,
                {
                  ...
                  dryAmounts: res.data.dryAmounts,
                  wetAmounts: res.data.wetAmounts,
                  sum: res.data.sum,
                },
              ]
            : []
