    ...
    >
        ...
        <el-form-item label-width="0" prop="id">
          <el-scrollbar height="200px">
            <el-checkbox-group v-model="form.resourceIds">
              <el-checkbox v-for="item in resourcesList" :key="item.rid" :label="item.rid">{{
                item.name
              }}</el-checkbox>
            </el-checkbox-group>
          </el-scrollbar>
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" :disabled="!form.resourceIds.length" @click="confirm">{{
            $t('message.hsdefine')
          }}</el-button>
        </span>
      </template>
        ...
        formRef.value.validate((valid) => {
          if (valid) {
            const obj: ClazzTheorySeting = {
              resourceIds: state.form.resourceIds,
