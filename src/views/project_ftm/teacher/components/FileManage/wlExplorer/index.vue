<template>
  ...
  <div class="wl-explorer" @contextmenu.prevent>
    ...
    <el-form class="wl-header-btn" :inline="true" :size="size" @submit.native.prevent>
      <el-form-item>
        <el-button type="primary" :disabled="pathIsStart" @click="pathBtn('prv')">{{
          $t('button.goBack')
        }}</el-button>
        <el-button type="primary" @click="handleFolder('add')" v-if="isAdd">{{
          $t('button.addFolder')
        }}</el-button>
        ...
        <submit-btn
          ...
          @btn="handleDel"
          :status="load.del"
          >{{ $t('button.delete') }}</submit-btn
        >
        <el-button @click="showUpload" :disabled="pathIsStart" v-if="isUpload">{{
          $t('button.uploadFolder')
        }}</el-button>
        <!-- solt自定义头部按钮区 -->
      ...
      <el-form-item>
        <el-dropdown
          trigger="click"
          placement="bottom"
          v-if="selfHeaderDropdown && selfHeaderDropdown.length"
    ...
    >
      <el-form-item class="file-path-box">
        ...
        >
          <img class="file-path-img" src="./images/folder@3x.png" :title="$t('common.folder')" />
          {{ file.path }}
        </div>
        <el-autocomplete
          class="u-full"
          ref="file-path-ipt"
          :placeholder="$t('holder.pleaseEnterFilePath')"
          v-model="file.path"
    ...
    <el-scrollbar class="wl-main-scroll">
      ...
      <div class="wl-main-list">
        ...
        <el-table
          ...
          ref="wl-table"
        >
          <el-table-column v-if="showCheckbox" align="center" type="selection" width="55" />
          <el-table-column
            v-if="showIndex"
            align="center"
            type="index"
            ...
            <template v-slot="scope">
              ...
              >
                ...
                <div class="namecol-iconbox">
                  <img :src="fileTypeIcon(scope.row)" class="name-col-icon" />
                </div>
                <!-- 不同文件类型 显示内容-->
        ...
        <ul class="wl-list" v-show="!layout.show_list">
          <li class="wl-list-item wl-is-folder" v-for="(i, idx) in self_data" :key="i.id">
            <el-checkbox
              class="wl-checkbox"
              @change="listItemCheck($event, i, idx)"
              :value="i._checked || false"
            />
            <div @click="enterTheLower(i, i[selfIsFolder])">
              <img :src="fileTypeIcon(i)" class="name-col-icon" />
              <p class="list-item-name" :title="i[selfProps.name]">
                {{ i.formatter ? i.formatter(i, null, i[selfProps.name], idx) : i[selfProps.name] }}
              </p>
            </div>
          </li>
        </ul>
    ...
    <!-- 移动文件区 -->
    <el-dialog :title="$t('common.moveFile')" width="520px" v-model="layout.move" center>
      <wlTreeSelect
        class="u-full"
        :size="size"
        :data="tree_path"
      ...
      />
      <template #footer>
        <el-button :size="size" @click="layout.move = false" type="primary" plain>{{
          $t('button.cancel')
        }}</el-button>
    ...
    <template v-if="useUpload">
      <fade-in v-show="layout.upload">
        ...
        <el-scrollbar class="scroll">
          ...
          >
            <el-form-item :label="$t('table.filePath')" v-if="tree_path && tree_path.length">
              <wlTreeSelect
                class="u-full"
                :size="size"
                :data="tree_path"
            ...
            <el-form-item :label="$t('table.exportFile')">
              <uploadItem
                ref="upload-item"
                :size="size"
                :reg="uploadReg"
...
<script>
  import submitBtn from './utils/submit-btn.vue' // 导入防抖组件
  ...
  import image_folder from './images/folder@3x.png'
  import image_file_img from './images/file_img@3x.png'
  import image_file_zip from './images/file_zip@3x.png'
  import image_file_video from './images/file_video@3x.png'
  import image_file_mp3 from './images/file_mp3@3x.png'
  import image_file_excel from './images/file_excel@3x.png'
  import image_file_docx from './images/file_docx@3x.png'
  import image_file_pdf from './images/file_pdf@3x.png'
  import image_file_ppt from './images/file_ppt@3x.png'
  import image_file_txt from './images/file_txt@3x.png'
    ...
    methods: {
      ...
      handleFolder(type) {
        ...
        if (type === 'edit' && (!_act || !_act[this.selfIsFolder])) {
          this.$message({
            showClose: true,
            message: this.$t('holder.pleaseSelectFolders'),
            type: 'error',
          })
          return
        }
      ...
      handleDel() {
        if (this.file_checked_data.length === 0) {
          this.$message({
            showClose: true,
            message: this.$t('holder.pleaseSelectDeleteFileOrFolder'),
            type: 'error',
          })
          return
        }
        // 删除确认
        this.$confirm(this.$t('tip.areYouSureYouWantToDelete'), {
          ...
          .catch(() => {
            this.$message({
              type: 'info',
              message: this.$t('tip.cancelledDelete'),
            })
          })
      },
      // 更多操作
      ...
      filePathChange(item) {
        if (!this.matched_path) {
          this.$confirm.alert(
            this.$t('tip.fileManagerCantFind') +
              this.file.path +
              this.$t('tip.checkSpellingTryAgain'),
            this.$t('table.fileExplorer'),
            {
              confirmButtonText: this.$t('button.confirm'),
              cancelButtonText: this.$t('button.cancel'),
              callback: () => {},
      ...
      routerPush(file, data = []) {
        ...
        this.self_data = data.map((v) => ({ _checked: false, ...v }))
      ...
      showMoveList() {
        if (this.file_checked_data.length === 0) {
          this.$message({
            showClose: true,
            message: this.$t('holder.pleaseSelectMoveFileOrFolder'),
            type: 'error',
          })
          return
        }
      ...
      wlDownload() {
        if (this.file_checked_data.length === 0) {
          this.$message({
            showClose: true,
            message: this.$t('holder.pleaseSelectDownloadFileOrFolder'),
            type: 'error',
          })
          return
        }
      ...
      uploadSuccess(res) {
        ...
        if (this.explorer_upload_data.bizId === this.file.id) {
          this.self_data.push(_res_data.map((v) => ({ _checked: false, ...v }))) // 当前文件夹上传 当即展示 因对象引用 历史记录也会自动更改
          return
        }
      ...
      fileTypeIcon(row) {
        ...
        if (row[this.selfIsFolder]) {
          _path = row[this.selfIsLock] ? image_file_automatic : image_folder
          return _path
        }
        // 其他根据后缀类型
        let _suffix = row[this.selfProps.suffix]
        if (!_suffix) {
          _path = image_file_none
          return _path
        }
        if (['jpg', 'jpeg', 'png', 'gif', 'bmp'].includes(_suffix)) {
          // 图片
          _path = image_file_img
        } else if (['zip', 'rar', '7z'].includes(_suffix)) {
          _path = image_file_zip
        } else if (['avi', 'mp4', 'rmvb', 'flv', 'mov', 'm2v', 'mkv', 'wmv'].includes(_suffix)) {
          _path = image_file_video
        } else if (['mp3', 'wav', 'wmv', 'wma'].includes(_suffix)) {
          _path = image_file_mp3
        } else if (['xls', 'xlsx'].includes(_suffix)) {
          _path = image_file_excel
        } else if (['doc', 'docx'].includes(_suffix)) {
          _path = image_file_docx
        } else if ('pdf' == _suffix) {
          _path = image_file_pdf
        } else if ('ppt' == _suffix || 'pptx' == _suffix) {
          _path = image_file_ppt
        } else if ('txt' == _suffix) {
          _path = image_file_txt
        } else {
          _path = image_file_none
        }
        return _path
      ...
      filrChecked(val) {
        ...
        val.forEach((item) => {
          let i = this.self_data.findIndex((v) => v.id == item.id)
          this.self_data[i]._checked = true
        })
        this.file_checked_data = val
      },
      // 列表模式记录多选数据
      listItemCheck(check, val, index) {
        this.self_data[index]._checked = check
        this.$nextTick(() => {
          this.self_data.forEach((row) => {
            this.$refs['wl-table'].toggleRowSelection(row, Boolean(row._checked))
          })
        })
      },
      // 清空关键词
      clearSearchKey() {
        this.file.key = ''
        this.self_data.forEach((v) => (v._checked = false))
      ...
      handleDataChange(val) {
        ...
        if (this.file.key) {
          this.self_data = _data.map((v) => ({ _checked: false, ...v }))
          return
        }
      ...
      closeViewEvent() {
        this.layout.view = false
        this.$emit('upload:previewOptions', '')
      },
    },
    computed: {
      // 自身头部更多操作自定义内容
