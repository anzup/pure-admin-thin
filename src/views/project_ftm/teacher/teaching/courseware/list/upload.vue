  ...
  export default {
    ...
    methods: {
      ...
      async handleSave() {
        let url = ''
        if (this.id) {
          url = `${import.meta.env.VITE_BASE_API_FTM}/coursewares/${this.id}`
        } else {
          url = `${import.meta.env.VITE_BASE_API_FTM}/coursewares`
        }
      ...
      postCoursewaresId() {
        ...
        let url = `${import.meta.env.VITE_BASE_API_FTM}/coursewares/${this.id}`
        let config = {
          headers: {
            'Content-Type': 'multipart/form-data',
            'Accept-Language': this.locale,
