import { postCoursewaresIdPlayProgress } from '/@/api/ftm/student/courseware'
/**
 *
 * 更新播放进度
 * @export
...
export const setLMSValue = function (CMIElement, value, progress) {
  ...
  if (CMIElement == 'cmi.completion_status' || CMIElement == 'cmi.core.lesson_status') {
    ...
    // 学习时间  PT14M37.63S
    let duration = 0
    const oldTime = sessionStorage.getItem('scormStartTime')
    if (oldTime && !Number.isNaN(new Date(oldTime).getTime())) {
      duration = Math.ceil((new Date() - new Date(oldTime)) / 1000)
    } else {
      duration = 0
    }
