...
export const getTrainingRecordDetail = function (groupId: number, type: TrainingRecordType) {
  return request<string[]>({
...
export const getTrainingRecordsProperties = function (type: TrainingRecordType) {
  return request<RecordsProperties[]>({
    url: url + `/trainingRecords/templates/${type}/properties`,
    method: 'get',
  })
}
export const putTrainingRecords = function (params: { type: TrainingRecordType; name: string }) {
...
export const getTrainingRecordsPdf = (groupId: number, type: string, portrait: boolean = false) => {
