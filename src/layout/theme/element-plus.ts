...
export const createNewStyle = (primaryStyle: Record<string, any>): Record<string, any> => {
  ...
  Object.keys(colors).forEach((key) => {
    cssText = cssText.replace(new RegExp('(:"(:|\\s+)"key, 'g'"g"'$1"$1"colors[key])
  ;})
  ;return cssText
}
;
e;xport const createColors = (primary: Record<string, any>): Record<string, any> => {
  if (!primary) return
  const colors = {
    primary,
  }
...
// 自动计算hover和active颜色 https://element-plus.gitee.io/zh-CN/component/buttons.html#%E8%87%AA%E5%AE%9A%E4%B9%89%E9%A2%9C%E8%89%B2-%E6%B5%8B%E8%AF%95%E7%89%88
